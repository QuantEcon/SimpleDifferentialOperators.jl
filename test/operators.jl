...
@testset "Operators under reflecting barrier conditions" begin
    ...
    @testset "Accuracy & regression test" begin
        ...
        @test @inferred(diffusionoperators(x̄, (Mixed(0.), Mixed(0.)))) == (L₁₋ = L₁₋, L₁₊ = L₁₊, L₂ = L₂, x = x) # test that the mixed case properly nests the reflecting case
    ...
    @testset "Consistency" begin
        ...
        L₁₋, L₁₊, L₂, x = diffusionoperators(uniformGrid,(Reflecting(), Reflecting()))
        L₁₋_ir, L₁₊_ir, L₂_ir, x_ir = diffusionoperators(irregularGrid, (Reflecting(), Reflecting()))
        @test L₁₋ ≈ L₁₋_ir
        @test L₁₊ ≈ L₁₊_ir
        @test L₂ ≈ L₂_ir
    end
...
@testset "Operators under mixed barrier conditions" begin
    ...
    @testset "Accuracy & regression test" begin
        ...
        @test Array(L₂) == [-2-1/(-1+ξ_lb) 1. 0.; 2/((2.0+1.0)*1.0) -2/(2.0*1.0) 2/((2.0+1.0)*2.0); 0. 1/4 -2*(1/(Δ_Mm*Δ_Mp)-1/((1+ξ_ub*Δ_Mp)*(Δ_Mp+Δ_Mm)*(Δ_Mp)))]
        @test Array(x) == [1.0; 2.0; 4.0]
    end
    ...
    @testset "Consistency" begin
        ...
        uniformGrid = range(0.0, 1.0, length = 500)
        irregularGrid = collect(uniformGrid)
        L₁₋, L₁₊, L₂, x = diffusionoperators(uniformGrid, (Mixed(ξ_lb), Mixed(ξ_ub)))
        L₁₋_ir, L₁₊_ir, L₂_ir, x_ir = diffusionoperators(irregularGrid, (Mixed(ξ_lb), Mixed(ξ_ub)))
        @test L₁₋ ≈ L₁₋_ir
        @test L₁₊ ≈ L₁₊_ir
        @test L₂ ≈ L₂_ir
        @test x ≈ x_ir
